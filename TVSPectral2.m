...
subNum = [1:length(files)]; % Creates array with subject numbers. Subject Numbers are STRINGS!
sesNum = [1:2]; % Number of sessions. [1 to #]
    ...
    for j = 1:length(subNum)
        for k = 1:length(sesNum)
            for l = 1:length(typeCond)
                ...
                if typeCond(l) == typeCond(1) % save Baseline data. Careful here! Baseline must come first and will be assigned to the following condition.
                    baseline = Engagement;
                elseif typeCond(l) == typeCond(2) % Write to Jessica Jones .xls file.
                    writePath = strcat(dataPath,'/P',string(subNum(j)),'_S',string(sesNum(k)),'_JJ_Engagement.xls');
                    xlswrite(writePath,baseline,1,'K4');
                    xlswrite(writePath,Engagement,1,'Z4');
                else % Write to Killer Women .xls file.
                    writePath = strcat(dataPath,'/P',string(subNum(j)),'_S',string(sesNum(k)),'_KW_Engagement.xls');
                    xlswrite(writePath,baseline,1,'K4');
                    xlswrite(writePath,Engagement,1,'Z4');
                end
            end
